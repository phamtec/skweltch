...
int main (int argc, char *argv[])
{
    ...
    desc.add_options()
		...
		("with", po::value<string>()->default_value("connect"), "bind or connect")
		("port", po::value<string>()->default_value("tcp://localhost:6666"), "The port.")
        ("dbHost", po::value<string>()->default_value("localhost"), "the location of the mongoDB.")
        ("resultsTable", po::value<string>(), "the database table where we can put results.")
    ;
    ...
    // minimal args
    if (vm.count("help") || !vm.count("with") || !vm.count("port")|| !vm.count("resultsTable")) {
	...
	while (1) {
		...
		// and write the results to the table.
		stringstream ss;
		ss << msg.getPayload();
		c.insert(vm["resultsTable"].as<string>(), mongo::fromjson(ss.str()));
